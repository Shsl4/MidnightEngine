...
void PerformanceWindow::render(const char* _errorText)
{
	...
	
	{
		...
		{
			((BGFXEngine*)BGFXEngine::getInstance())->restart();
		}
		...
		{
			((BGFXEngine*)BGFXEngine::getInstance())->stop();
		}
...

	{
		...
		{
			BGFXEngine::renderer = supportedRenderers[current + 1];
			((BGFXEngine*)BGFXEngine::getInstance())->restart();
		}
		...
		if (num) {
			...
			{
				((BGFXEngine*)BGFXEngine::getInstance())->selectGPU(caps->gpu[current].deviceId, caps->gpu[current].vendorId);
				((BGFXEngine*)BGFXEngine::getInstance())->restart();
			}
