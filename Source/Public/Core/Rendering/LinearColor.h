...
struct ENGINE_API LinearColor {
    ...
    FORCEINLINE float getRed() const {
        return this->r;
    }

    FORCEINLINE float getGreen() const {
        return this->g;
    }

    FORCEINLINE float getBlue() const {
        return this->b;
    }
    ...
    FORCEINLINE void setRed(float red) {
        this->r = Math::clamp(red, 0.0f, 1.0f);
    }

    FORCEINLINE void setGreen(float green) {
        this->g = Math::clamp(green, 0.0f, 1.0f);
    }

    FORCEINLINE void setBlue(float blue) {
        this->b = Math::clamp(blue, 0.0f, 1.0f);
    }
